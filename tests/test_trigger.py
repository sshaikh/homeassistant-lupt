...
def help_test_next_time(hass, prayer, delta, dt, expected):
    """Help test next time."""
    listener = LuptListener(hass, None, prayer, delta)
...
async def fire_time(hass, patched_time):
...
async def test_track_prayer(hass, legacy_patchable_time, lupt_mock):
    """Track a lupt event."""
    ...
    await fire_time(hass, create_utc_datetime(2021, 10, 2, 5, 25))
    assert len(runs) == 0
    assert len(offset_runs) == 0

    await fire_time(hass, create_utc_datetime(2021, 10, 2, 5, 35))
    assert len(runs) == 0
    assert len(offset_runs) == 1

    await fire_time(hass, create_utc_datetime(2021, 10, 2, 6, 0))
    assert len(runs) == 1
    assert len(offset_runs) == 1

    off_listener.async_detach()

    await fire_time(hass, create_utc_datetime(2021, 10, 3, 5, 50))
    assert len(runs) == 1
    assert len(offset_runs) == 1

    listener.async_detach()

    await fire_time(hass, create_utc_datetime(2021, 10, 3, 7, 5))
    assert len(runs) == 1
    assert len(offset_runs) == 1
